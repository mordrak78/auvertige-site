...
import { Heart, Gift, Flower, Leaf, Palette, Star, Truck } from 'lucide-react';

const imagesPlaisirsOffrir = [
  ...
  images.creations.plaisirOffrir.roses3,
  images.creations.plaisirOffrir.roses4,
  images.creations.plaisirOffrir.roses5,
  images.creations.plaisirOffrir.roses6,
  images.creations.plaisirOffrir.roses7,
  images.creations.plaisirOffrir.bouquet1,
  images.creations.plaisirOffrir.bouquet2,
  images.creations.plaisirOffrir.bouquet3,
  images.creations.plaisirOffrir.bouquet4,
];

const PlaisirsOffrir = () => {
  useScrollToTop();
    ...
    <Layout>
      <Seo
        ...
        breadcrumbs={[
          ...
          { name: "Plaisir d'offrir", url: "/plaisirs-offrir" }
        ]}
      />
      <section className="min-h-screen bg-cream-50 py-16">
        <div className="container mx-auto px-4 max-w-4xl">
          <h1 className="text-4xl md:text-5xl font-dancing text-sage-700 mb-8 text-center">Plaisir d'offrir</h1>
          <p className="text-lg text-sage-600 mb-8 text-center max-w-2xl mx-auto">
            Des bouquets et compositions pour toutes les occasions de faire plaisir à vos proches, livrés à Nantes Sud, quartier Saint-Jacques, Pirmil, Rezé ou Saint-Sébastien-sur-Loire. Créations personnalisées, conseils sur-mesure, livraison rapide.
          </p>
          <div className="flex flex-col sm:flex-row justify-center gap-4 mb-10">
            <Link to="/contact" className="bg-poppy-500 hover:bg-poppy-600 text-white font-bold py-3 px-6 rounded-full shadow transition text-lg">Commander un bouquet</Link>
          ...
          {/* Section Galerie Plaisirs Offrir */}
          <section className="mb-16">
            <h3 className="text-xl md:text-2xl font-dancing text-sage-700 mb-6 text-center">Découvrez nos réalisations</h3>
            <Carousel opts={{ align: 'start' }}>
              <CarouselContent>
                {Array.from({ length: Math.ceil(imagesPlaisirsOffrir.length / 4) }).map((_, slideIdx) => (
                  <CarouselItem key={slideIdx}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                      {imagesPlaisirsOffrir.slice(slideIdx * 4, slideIdx * 4 + 4).map((img, idx) => (
                        <div key={img} className="bg-white rounded-2xl shadow-md overflow-hidden">
                          <img src={img} alt={`Plaisir d'offrir ${slideIdx * 4 + idx + 1}`} className="w-full aspect-[9/16] object-cover object-center" loading="lazy" />
                        </div>
                      ))}
                    </div>
                  </CarouselItem>
          ...
          <section className="mb-16">
            <h2 className="text-2xl md:text-3xl font-semibold text-sage-700 mb-4">Nos prestations plaisir d'offrir</h2>
            <ul className="text-sage-600 text-lg space-y-2 mb-6">
              <li>💝 Bouquets personnalisés pour toutes les occasions</li>
              <li>🎁 Livraison à domicile ou sur le lieu de votre choix</li>
